...
class WPBakeryShortCode_borderless_wpbakery_svg extends WPBakeryShortCode {
    protected function content( $atts, $content = null ) {
        ...
        $svg_src = isset( $svg_img[0] ) ? $svg_img[0] : '';
        ...
        $svg_src   = esc_url( $svg_src );
        $css_class = esc_attr( $css_class );

        $output .= '<div ' . $el_id . ' class="borderless-wpbakery-svg ' . $css_class . ' ' . $alignment . '">';
        $output .= '<div class="borderless-wpbakery-svg-inner" style="height:' . $height . ';width:' . $width . ';"><img class="borderless-svg-img" src="' . $svg_src . '"/></div>';
        $output .= '</div>';
